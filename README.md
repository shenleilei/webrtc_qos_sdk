...
`scripts/verify_webrtc_first_phase2_completion_audit.sh` 是“能否宣布 Phase-2 完成”的审计门禁。它不会重新跑所有 case，而是检查既有证据是否达到完成标准：smoke/qoe 必须通过，production soak 必须来自 `SOAK_MINUTES>=120` 或更长的正式运行，并通过 `scripts/verify_webrtc_first_qoe_production_soak_evidence.sh` 离线复验 summary/CSV/config/archive、clean tracked worktree metadata、summary 与 CSV 聚合一致性、弱网低发送预算和恢复时间分布；real renderer 必须是有真实显示环境的 pass 结果，并通过 `scripts/verify_real_renderer_evidence.sh` 离线复验 summary/metrics、非 Xvfb backend、实际 rendered frames、late/gap/jitter 预算；capture library 必须是正式业务素材库而不是 fixture，并通过 `scripts/verify_capture_library_evidence.sh` 绑定复验 manifest summary、QoE CSV 和 QoE summary 中的 `capture_manifest_sha256` 与 `capture_media_sha256`。审计同时输出 `phase2_completion_audit_summary.txt` 和 `phase2_completion_audit_metrics.prom`；后者用 Prometheus/textfile 形式导出 audit/completion status、pass/warn/fail check 数、单项 check 状态，以及 production soak、real renderer、capture library、evidence bundle 的生产证据状态，供 P5 production gate 和监控系统离线复验。当前本机预期不会通过这个审计，因为还缺多小时 production soak、真实 GPU/窗口 renderer 和正式业务采集素材库。

`scripts/collect_webrtc_first_phase2_evidence_bundle.sh` 用于把一次正式验收的 smoke/qoe/production soak/real renderer/capture 证据收集到统一目录，并生成 `files.txt` 和 `manifest.sha256`。bundle 的 `metadata.env` 会记录 `GIT_HEAD` 和 `GIT_TRACKED_WORKTREE_CLEAN`；P5 外部导入要求它来自 clean tracked worktree，并会复验 `files.txt`、`manifest.sha256` 和实际文件集合一致性。后续可以用 `EVIDENCE_BUNDLE_DIR=/path/to/bundle scripts/verify_webrtc_first_phase2_completion_audit.sh` 直接审计该目录，避免正式结果散落在多个 artifacts 路径里。

`scripts/verify_phase5_production_readiness.sh` 是 Phase-5 正式验收前置检查：它不跑长时 soak，只检查 tracked worktree 是否干净、WebRTC module prefix、`SOAK_MINUTES` 配置、正式 capture manifest、真实 renderer 可用性和 Phase-5 gate/audit 脚本是否齐全，并输出 `phase5_production_readiness_status=ready|not_ready`、logs、`readiness_report.json`、`next_required_actions.json`、`risk_milestone_report.json`、`risk_milestone_summary.txt`、`phase5_production_readiness_metrics.prom`、`next_required_actions.txt`、`files.txt` 和 `manifest.sha256`。JSON 报告会记录每个 check、失败/跳过原因、机器可消费的 remediation action、M1-M6 里程碑状态和 R1-R5 风险状态；`.prom` 文件是 Prometheus/textfile 指标出口，用于把 readiness 状态、失败/跳过/action 数、M1-M6、R1-R5 和 remediation action 导入 CI/监控告警；其中 M6 fanout 在 P5 基础范围内固定为 deferred，正式完成状态仍要求 passed Phase-5 production gate。文本文件用于人工排查。readiness 也会拒绝把 `MIN_PRODUCTION_SOAK_MINUTES` 降到 `120` 以下；直接调用 `import_phase5_phase2_evidence_bundle.sh` 或 Phase-2 completion audit 时也保留同一最低值，不能绕过顶层 production gate 降级正式 soak 要求。设置 `PHASE2_EVIDENCE_BUNDLE_DIR` 时，readiness 只有在外部导入报告 `import_status=pass`、clean tracked worktree、git head 匹配、长稳/真实 renderer/capture QoE 原始证据以及 capture manifest/media sha256 都通过复验后，才会把 `capture_manifest` 和 `real_renderer` 标为来自外部 bundle 的 pass；同时会在导入报告上复验 capture fixture flag、QoE `rows/pass_rows` 完整性、必需类别覆盖、`playable_ratio / avg_psnr_y / avg_ssim_y` 下限、`decode_errors / freeze_count / renderer_proxy_drop_frames` 全 0，以及 QoE summary 中的 `capture_manifest_sha256` 和 `capture_media_sha256` 必须等于 manifest summary。默认本地缺正式素材或真实 renderer 时只生成 not-ready 报告；正式 CI 可设置 `REQUIRE_READY=1` 把这些缺口变成硬失败。

`scripts/run_phase5_production_gate.sh` 是 Phase-5 顶层正式验收 wrapper。它先跑并复验 Phase-5 implementation gate，再跑 release contract、production readiness 和 debug bundle 门禁，然后调用 `run_webrtc_first_phase2_production_gate.sh` 做 production preflight、长时 soak、真实 renderer、正式 capture library、evidence bundle 和 completion audit，并在 `artifacts/phase5_production_gate/<utc_build_id>/` 下生成 metadata、summary、logs、`git_tracked_status.txt`、`phase5_release_evidence.json`、`phase5_release_evidence.txt`、`phase5_production_gate_metrics.prom`、`files.txt` 和 `manifest.sha256`；其中 `phase5_implementation_gate/` 固定在 production gate 目录内，保证正式证据自包含。release evidence 是正式发布证据索引，必须同时指向顶层 `files.txt`、顶层 `manifest.sha256`、顶层 `phase5_production_gate_metrics.prom`、release evidence JSON/summary 自身、implementation gate、implementation gate `.prom` 指标、clean tracked worktree、readiness summary、readiness report、next required actions、risk milestone report、readiness `.prom` 指标、readiness check records、debug bundle manifest、runtime config、health/SLO report、monitoring metrics、alert policy、incident report/runbook、timeline、first problem、alerts summary、底层 Phase-2 production gate、Phase-2 completion audit `.prom` 指标、production soak 原始 summary/CSV/archive、真实 renderer summary/metrics、正式 capture library、capture manifest summary、capture manifest sha256、capture media sha256、capture QoE CSV、capture QoE summary、evidence bundle 和 completion audit，并写出 `min_production_soak_minutes`、`phase5_gate_files`、`phase5_gate_manifest`、`phase5_production_gate_metrics`、`phase5_release_evidence_json`、`phase5_release_evidence_summary`、`production_soak_rows`、`real_renderer_status`、`real_renderer_backend`、`capture_manifest_sha256`、`capture_media_sha256`、`phase5_readiness_report`、`phase5_readiness_metrics`、`phase5_risk_milestone_report`、`phase5_debug_health_report`、`phase5_debug_monitoring_metrics`、`phase5_debug_incident_report` 和 `capture_qoe_minima` 便于排障；release evidence verifier 会按固定 evidence id 集合校验，包括 release evidence JSON/summary 自身，重复 id 或未知 id 都会失败。顶层 `.prom` 文件输出 gate pass/fail/dry_run、step 状态、failure debug bundle 状态、release evidence 状态和 release evidence item pass/fail/missing/invalid 计数，便于 CI/监控直接告警。顶层 Phase-5 gate 和底层 Phase-2 production gate 都把 `MIN_PRODUCTION_SOAK_MINUTES` 默认固定为 `120`，并在入口前置拒绝 `SOAK_MINUTES<120`、`MIN_PRODUCTION_SOAK_MINUTES<120` 或 `SOAK_MINUTES<MIN_PRODUCTION_SOAK_MINUTES` 的配置，避免短时 production run 先消耗测试机再由末端审计判无效；release evidence verifier 还会确认声明最低值不低于 120，且实际 production soak 分钟数不低于声明最低值，并确认 release evidence JSON/summary 自身已经纳入最终顶层 `manifest.sha256`。release evidence 生成阶段也会拒绝 `renderer_backend=xvfb` 或没有实际 rendered frames 的 renderer 结果，不能先声明弱 renderer 证据为 pass 再交给离线 verifier 打回；同一阶段还会拒绝 fixture capture manifest，以及行数不完整、类别覆盖不足、QoE 下限缺失、`decode_errors / freeze_count / renderer_proxy_drop_frames` 非 0 或 QoE summary 未绑定 manifest/media sha 的 capture 证据。真实 renderer/正式素材/长时 soak 如果在专用测试机完成，也可以先收集 Phase-2 evidence bundle，再通过 `PHASE2_EVIDENCE_BUNDLE_DIR=/path/to/phase2_evidence_bundle scripts/run_phase5_production_gate.sh` 导入；导入路径会运行 `import_phase5_phase2_evidence_bundle.sh`，复验源 bundle、复制后 bundle 和导入目录的 `files.txt`/`manifest.sha256`/实际文件集合一致性，以及 Phase-2 completion audit、Phase-2 completion audit `.prom` 指标、git head、bundle clean tracked worktree、production soak evidence log、真实 renderer、正式 capture library manifest、capture manifest sha256、capture media sha256 和 `webrtc_first_qoe_capture_library_720p.csv`，并在导入报告里记录 production soak summary/CSV/config/archive/evidence log、真实 renderer summary/metrics、capture manifest summary、capture manifest/media sha256、capture QoE CSV/summary 的原始证据指针；production gate 会用 `verify_webrtc_first_qoe_production_soak_evidence.sh` 离线复验 production soak 指针，用 `verify_real_renderer_evidence.sh` 离线复验真实 renderer 指针，并用 `verify_capture_library_evidence.sh` 离线复验 capture manifest/QoE 绑定，不能用 fixture、不匹配代码版本、dirty 源码证据、短时 soak、summary/CSV 不一致、Xvfb-only renderer、未实际 rendered frames、未绑定 manifest/media sha 的 QoE summary 或只有 manifest 没有 QoE 结果的素材库绕过。非 dry-run 执行失败时，wrapper 会自动收集并校验 `failure_debug_bundle/`，确保失败也有日志、metrics、alerts、timeline 和 runtime config 可排查；`verify_phase5_production_gate.sh` 会在成功路径离线复验 `phase5_implementation_gate/`、`phase5_production_readiness/`、clean tracked worktree 证据、`phase5_debug_bundle/`、`phase5_release_evidence.json`，同时复验顶层 `files.txt` 与 `manifest.sha256` 文件集合一致、production gate `.prom` 指标、implementation gate `.prom` 指标、readiness `.prom` 指标、底层 Phase-2 evidence bundle `files.txt`/`manifest.sha256`/实际文件集合一致性、`phase2_completion_audit=pass / phase2_completion_status=complete`、`phase2_completion_audit_metrics.prom`、production soak archive/evidence、真实 renderer metrics、release evidence 里索引的顶层 gate manifest/files/metrics、自身 JSON/summary、readiness report/actions/risk milestone/check records、debug bundle health/SLO/monitoring/alert policy/incident/timeline 证据、capture manifest/media sha256 和 capture QoE CSV，在失败路径复验 `failure_debug_bundle/`，并且只要 implementation gate 已经 pass，也会复验该实现证据；如果失败发生在 implementation/readiness 阶段，还会复验对应 summary、manifest、失败 check、implementation gate `.prom` 指标、`readiness_report.json`、`next_required_actions.json`、`risk_milestone_report.json`、`phase5_production_readiness_metrics.prom` 和 `next_required_actions.txt`，避免只相信 summary 或留下不可执行的失败报告。底层 Phase-2 production gate 默认要求 `SOAK_MINUTES=120`。当前本机 preflight 预期仍会失败：`/root/webrtc_qos_sdk/capture_library/manifest.csv` 不存在，且没有真实 renderer 环境；可以用 `PHASE5_DRY_RUN=1` 只验证 gate 结构，但这不代表生产证据完成。
...
`verify_phase5_error_contract.sh` 是 Phase-5 错误码与运行契约门禁：它先安装当前 SDK，再用外部 CMake consumer 只链接 `WebRtcQosSdk::role_push / role_play / role_server`，验证 `Start()` config error、显式启用 log/metrics/alerts 但文件输出不可写、before-start 调用、malformed H264/RTP、transport output failure、server relay failure 和 decode output failure 的 `StatusCode`、日志事件和 alerts 规则一致。

`collect_phase5_debug_bundle.sh` / `verify_phase5_debug_bundle.sh` 是 Phase-5 排障包门禁：collector 默认跑一次 UDP selftest，同时开启 `--log-dir / --metrics-dir / --alerts-dir`，把 metadata、build config、git status、session config、runtime config dump、push/server/play 日志、metrics、alerts、timeline、first problem、metrics/alerts/timeline 汇总、`monitoring/health_report.json`、`monitoring/health_summary.txt`、`monitoring/slo_report.json`、`monitoring/slo_summary.txt`、`monitoring/phase5_monitoring_metrics.prom`、`monitoring/alert_policy.json`、`monitoring/alert_policy_summary.txt`、`monitoring/incident_report.json`、`monitoring/incident_runbook.txt`、`files.txt` 和 sha256 manifest 收集到一个目录；其中 `.prom` 文件是 Prometheus/textfile 风格指标，便于 CI、监控系统或值班工具直接导入单次排障包的 role 健康、alert、SLO 和 policy 观测值；verifier 离线校验必需文件、JSON 字段、Prometheus 文本格式、每个 role 的 `config_dump` 日志脱敏快照、弱网告警规则、首个告警/首个问题汇总、timeline、health report recommended actions、SLO/SLA 目标与观测值、默认 alert policy、incident runbook、`files.txt`/`manifest.sha256`/实际文件集合一致性、runtime config 脱敏标记，以及 bundle 中不能出现 payload/token/secret/password 类字段。
...
`run_phase5_implementation_gate.sh` 是 Phase-5 非生产实现证据 wrapper：它串起 no-selfmade、logging、metrics、alerts、error contract、minimal UDP external app、release contract 和 debug bundle 门禁，并在 `artifacts/phase5_implementation_gate/<utc_build_id>/` 下保留 summary、logs、关键运行产物、`phase5_implementation_gate_metrics.prom`、`files.txt` 和 `manifest.sha256`。顶层 `.prom` 文件输出 gate pass/fail、各 step 最终状态和 debug bundle 状态，便于 CI/监控在正式 production gate 前先判断实现证据是否闭合。`verify_phase5_implementation_gate.sh` 会离线复验所有子门禁 pass、三角色日志/metrics/alerts 产物、debug bundle manifest、顶层 `.prom` 指标、`files.txt`/`manifest.sha256`/实际文件集合一致性和运行 JSON 统一身份字段，避免 completion audit 只相信脚本存在。

`verify_phase5_completion_audit.sh` 是 Phase-5 完成度审计入口：默认要求传入已通过的 `PHASE5_GATE_DIR` 并验证正式 production evidence；如果该 production gate 内包含 `phase5_implementation_gate/`，audit 会自动复验实现证据，也可以显式传 `PHASE5_IMPLEMENTATION_GATE_DIR`。正式 completion 还会要求 production gate `metadata.txt` 和 `phase5_release_evidence.json` 中的 `GIT_HEAD` 等于当前 checkout HEAD，避免把旧 commit 的生产证据拿来宣布当前源码完成。审计会输出 `phase5_completion_audit_summary.txt` 和 `phase5_completion_audit_metrics.prom`，其中 Prometheus/textfile 指标覆盖 completion status、audit status、pass/warn/fail check 数、单项 check 状态、production evidence 状态和 next required action，便于 CI/监控直接区分“正式完成”和“只缺生产证据”。本地可用 `REQUIRE_PRODUCTION_EVIDENCE=0` 审计“除正式生产证据外的 P5 实现项是否齐全”，但仍需要 implementation gate 证据，且不会把 P5 判定为生产完成。
...
真实 renderer 不能继续和 renderer proxy 混为一谈。已新增 `scripts/verify_real_renderer_smoke.sh`：有 `DISPLAY` 且可链接 X11 时，它会创建真实 X11 window，按 30fps present 帧并输出 `rendered_frames / late_frames / avg_present_gap_ms / max_present_gap_ms / avg_present_jitter_ms / max_present_jitter_ms`；没有 `DISPLAY` 但存在 `Xvfb` 时，脚本会自动启动 headless X11 server 跑同一套 X11 present smoke，用于 CI 覆盖窗口 present 代码路径；两者都不可用时默认写出 skipped 证据，设置 `REQUIRE_REAL_RENDERER=1` 则直接失败。`scripts/verify_real_renderer_evidence.sh` 用于对正式 summary/metrics 做离线复验：要求 `real_renderer_status=pass`、非 Xvfb backend、`rendered_frames>=1` 且与 `frames` 一致，并复验 late frames、present gap 和 present jitter 预算。当前机器没有 `DISPLAY/WAYLAND_DISPLAY`，也没有 `Xvfb`，所以本地结果是：
...
这说明当前 QoE 结论仍以 renderer proxy 播放调度为准，不能声明已经完成真实 GPU/窗口 renderer 验收；但真实 renderer/Xvfb renderer 的可选门禁入口和正式证据离线复验入口已经具备。
...
这个审计脚本会读取 smoke/qoe summary、production soak summary/config/archive、real renderer summary/metrics 和 capture manifest summary，并复用 `verify_webrtc_first_qoe_production_soak_evidence.sh` 校验 sha256 manifest、summary/CSV 一致性、`SOAK_MINUTES>=120`、row pass、弱网低 RPS/低码率和恢复时间分布，也复用 `verify_real_renderer_evidence.sh` 校验真实 renderer status/backend/rendered frames/late/gap/jitter。它的默认 production 输入路径是 `artifacts/webrtc_first_phase2_verify_production/`，也可以通过 `EVIDENCE_BUNDLE_DIR` 审计收集好的 bundle；bundle 审计会同时复验 `files.txt`、`manifest.sha256` 和实际文件集合一致性，避免外部证据目录漏列或混入文件。每次执行都会写出 `phase2_completion_audit_metrics.prom`，即使失败也会暴露缺失证据的机器可读状态。当前只有短时 production smoke、renderer skipped 和 fixture capture 时会失败，这是预期结果；失败 summary 会写出还缺哪类证据。
...
manifest 至少需要 `category,path` 两列，可选 `label,enabled`。默认要求六类正式素材都存在且文件路径不能重复冒充多类：`indoor_face / outdoor_walking / low_light_noise / screen_text / high_motion / scene_cut`。视频文件会用 `ffprobe` 检查存在视频流和最小时长；raw `.yuv/.i420` 会按 `CAPTURE_WIDTH x CAPTURE_HEIGHT x MIN_CAPTURE_FRAMES` 检查大小。校验 summary 会写出 `capture_manifest_sha256` 和对启用素材文件内容聚合后的 `capture_media_sha256`，QoE runner 会把同一组 sha 写入 `capture_qoe_summary.txt`，`scripts/verify_capture_library_evidence.sh` 会离线复验 manifest summary、QoE CSV 和 QoE summary 三者一致。Phase-2 completion audit、外部 evidence bundle import 和 Phase-5 release evidence 都会携带并复验这些哈希，避免正式素材库 manifest 或媒体文件被替换后只靠旧 summary 或旧 CSV 通过；旧的 capture QoE summary 若没有 `capture_manifest_sha256` 或 `capture_media_sha256`，需要重新跑 capture library runner 后才能作为正式证据。完整 QoE 跑法可以打开强门禁：
