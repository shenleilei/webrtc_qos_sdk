...
- P5 默认按无显卡/无真实生产素材环境执行：`P5_SKIP_REAL_RENDERER=1` 和 `P5_SKIP_CAPTURE_LIBRARY=1` 会记录显式 policy skip 证据；仍要求 `SOAK_MINUTES>=120` 的 production soak。正式 P5 soak 默认覆盖 `baseline / weak_network_low_rps_low_bitrate` 和 `block_motion / camera_pan / scene_cut / low_light_noise`；`walking_dead_zone_recover / oscillating_edge_recover` 保留为扩展 stress，不作为 P5 完成 gate 默认集合。若要做严格实机/业务素材验收，把这两个变量设为 `0`。
...
这个脚本已经具备生产级多轮/多分钟 soak 的执行入口和归档/验签入口；真正的“多小时”结论还需要在稳定机器上用 `SOAK_MINUTES=120` 或更长时间实际跑完，并把 tarball、summary 和日志归档。若要把扩展 stress 纳入专项验收，可以显式追加 `SCENARIOS="baseline weak_network_low_rps_low_bitrate walking_dead_zone_recover oscillating_edge_recover"`，但这不属于 P5 默认完成门禁。
...
正式命令默认使用 `PRODUCTION_SOAK_SCENARIOS="baseline weak_network_low_rps_low_bitrate"`；只有做扩展 stress 时才显式追加 dead-zone/oscillating 场景。
