...
- P5 默认按无显卡/无真实生产素材环境执行：`P5_SKIP_REAL_RENDERER=1` 和 `P5_SKIP_CAPTURE_LIBRARY=1` 会记录显式 policy skip 证据；仍要求 `SOAK_MINUTES>=10` 的 production soak，用于短长稳和证据闭环。正式 P5 soak 默认覆盖 `baseline / weak_network_low_rps_low_bitrate` 和 `block_motion / camera_pan / scene_cut / low_light_noise`；`walking_dead_zone_recover / oscillating_edge_recover` 保留为扩展 stress，不作为 P5 完成 gate 默认集合。若要做严格实机/业务素材验收，把这两个变量设为 `0`，并显式使用 `SOAK_MINUTES=120 MIN_PRODUCTION_SOAK_MINUTES=120`。
...
`verify_webrtc_first_phase2.sh` 是当前 Phase-2 聚合门禁入口。`VERIFY_LEVEL=smoke` 会串起 no-selfmade、WebRTC module smoke、外部 CMake package、loopback、pacing probe、role facade 和 synthetic 弱网矩阵；`VERIFY_LEVEL=qoe` 在 smoke 基础上增加低 RPS/低码率真实 H264 QoE 和恢复时间分布；`VERIFY_LEVEL=production` 会继续进入 production soak，并可通过 `REQUIRE_REAL_RENDERER=1 / REQUIRE_CAPTURE_LIBRARY=1` 把真实 renderer 和正式采集素材库变成硬门禁。当前 `SOAK_MINUTES=0` 的默认本地 production smoke 只验证 runner/archive 链路，默认配置为 `FRAMES_PER_CYCLE=12 / CONTENT_MODES=block_motion / SCENARIOS=weak_network_low_rps_low_bitrate`；P5 默认正式门槛是 `SOAK_MINUTES>=10`，严格生产验收可显式设置 `SOAK_MINUTES=120 MIN_PRODUCTION_SOAK_MINUTES=120`。正式 P5 soak 的默认稳定性集合是 `PRODUCTION_SOAK_SCENARIOS="baseline weak_network_low_rps_low_bitrate"`、`PRODUCTION_SOAK_CONTENT_MODES="block_motion camera_pan scene_cut low_light_noise"`、`PRODUCTION_SOAK_SEEDS=1`。`walking_dead_zone_recover` 和 `oscillating_edge_recover` 仍可通过 `SCENARIOS` 或 `PRODUCTION_SOAK_SCENARIOS` 显式加入，用于扩展 stress/专项排查；它们可能触发严格 renderer proxy gap/drop 门槛，因此不作为 P5 完成 gate 默认集合。

`scripts/verify_webrtc_first_phase2_completion_audit.sh` 是“能否宣布 Phase-2 完成”的审计门禁。它不会重新跑所有 case，而是检查既有证据是否达到完成标准：smoke/qoe 必须通过，production soak 必须来自 `SOAK_MINUTES>=10` 或更长的正式运行，并通过 `scripts/verify_webrtc_first_qoe_production_soak_evidence.sh` 离线复验 summary/CSV/config/archive、clean tracked worktree metadata、summary 与 CSV 聚合一致性、弱网低发送预算和恢复时间分布。P5 默认允许 `P5_SKIP_REAL_RENDERER=1` / `P5_SKIP_CAPTURE_LIBRARY=1`，在没有显卡/显示环境或没有真实生产素材时生成 `skipped_by_policy` 证据并作为 P5 范围内的 pass；如果把 skip 设为 `0`，real renderer 必须是有真实显示环境的 pass 结果，并通过 `scripts/verify_real_renderer_evidence.sh` 离线复验 summary/metrics、非 Xvfb backend、实际 rendered frames、late/gap/jitter 预算，capture library 必须是正式业务素材库而不是 fixture，并通过 `scripts/verify_capture_library_evidence.sh` 绑定复验 manifest summary、QoE CSV 和 QoE summary 中的 `capture_manifest_sha256` 与 `capture_media_sha256`。审计同时输出 `phase2_completion_audit_summary.txt` 和 `phase2_completion_audit_metrics.prom`；后者用 Prometheus/textfile 形式导出 audit/completion status、pass/warn/fail check 数、单项 check 状态，以及 production soak、real renderer、capture library、evidence bundle 的生产证据状态，供 P5 production gate 和监控系统离线复验。
...
`scripts/verify_phase5_production_readiness.sh` 是 Phase-5 正式验收前置检查：它不跑长时 soak，只检查 tracked worktree 是否干净、WebRTC module prefix、`SOAK_MINUTES` 配置、可选 capture manifest、可选 real renderer 和 Phase-5 gate/audit 脚本是否齐全，并输出 `phase5_production_readiness_status=ready|not_ready`、logs、`readiness_report.json`、`next_required_actions.json`、`risk_milestone_report.json`、`risk_milestone_summary.txt`、`phase5_production_readiness_metrics.prom`、`next_required_actions.txt`、`files.txt` 和 `manifest.sha256`。默认 `P5_SKIP_REAL_RENDERER=1`、`P5_SKIP_CAPTURE_LIBRARY=1`，没有显卡/显示环境或没有真实生产素材时会把 `real_renderer` / `capture_manifest` 记为 `policy=skipped_by_p5_*` 的 pass，并写入 log、JSON 和 manifest；设置为 `0` 时恢复硬门禁。JSON 报告会记录每个 check、失败/跳过原因、机器可消费的 remediation action、M1-M6 里程碑状态和 R1-R5 风险状态；`.prom` 文件是 Prometheus/textfile 指标出口，用于把 readiness 状态、失败/跳过/action 数、M1-M6、R1-R5 和 remediation action 导入 CI/监控告警；其中 M6 fanout 在 P5 基础范围内固定为 deferred，正式完成状态仍要求 passed Phase-5 production gate。文本文件用于人工排查。readiness 也会拒绝把 `MIN_PRODUCTION_SOAK_MINUTES` 降到 `10` 以下；直接调用 `import_phase5_phase2_evidence_bundle.sh` 或 Phase-2 completion audit 时也保留同一最低值，不能绕过顶层 production gate 降级正式 soak 要求。

`scripts/run_phase5_production_gate.sh` 是 Phase-5 顶层正式验收 wrapper。它先跑并复验 Phase-5 implementation gate，再跑 release contract、production readiness 和 debug bundle 门禁，然后调用 `run_webrtc_first_phase2_production_gate.sh` 做 production preflight、长时 soak、可选真实 renderer、可选 capture library、evidence bundle 和 completion audit，并在 `artifacts/phase5_production_gate/<utc_build_id>/` 下生成 metadata、summary、logs、`git_tracked_status.txt`、`phase5_release_evidence.json`、`phase5_release_evidence.txt`、`phase5_production_gate_metrics.prom`、`files.txt` 和 `manifest.sha256`；其中 `phase5_implementation_gate/` 固定在 production gate 目录内，保证正式证据自包含。P5 当前默认 `P5_SKIP_REAL_RENDERER=1`、`P5_SKIP_CAPTURE_LIBRARY=1`，用于无显卡/显示环境、无真实生产素材库时显式跳过这两项并写入 `skipped_by_policy` 证据；如果要恢复严格实机/业务素材验收，把两个变量设为 `0`。顶层 Phase-5 gate 和底层 Phase-2 production gate 都把 `MIN_PRODUCTION_SOAK_MINUTES` 默认固定为 `10`，并在入口前置拒绝 `SOAK_MINUTES<10`、`MIN_PRODUCTION_SOAK_MINUTES<10` 或 `SOAK_MINUTES<MIN_PRODUCTION_SOAK_MINUTES` 的配置；严格生产验收可显式设为 `120`。P5 默认 production soak 只把 `baseline` 和 `weak_network_low_rps_low_bitrate` 作为正式稳定性场景，覆盖四类 deterministic 内容；dead-zone/oscillating edge 是可显式开启的扩展 stress，不默认参与完成 gate。release evidence verifier 会按固定 evidence id 集合校验，包括 release evidence JSON/summary 自身，重复 id 或未知 id 都会失败；也会确认 release evidence JSON/summary 自身已经纳入最终顶层 `manifest.sha256`。顶层 `.prom` 文件输出 gate pass/fail/dry_run、step 状态、failure debug bundle 状态、release evidence 状态和 release evidence item pass/fail/missing/invalid 计数，便于 CI/监控直接告警。非 dry-run 执行失败时，wrapper 会自动收集并校验 `failure_debug_bundle/`，确保失败也有日志、metrics、alerts、timeline 和 runtime config 可排查；`verify_phase5_production_gate.sh` 会在成功路径离线复验 `phase5_implementation_gate/`、`phase5_production_readiness/`、clean tracked worktree 证据、`phase5_debug_bundle/`、`phase5_release_evidence.json`，同时复验顶层 `files.txt` 与 `manifest.sha256` 文件集合一致、production gate `.prom` 指标、implementation gate `.prom` 指标、readiness `.prom` 指标、底层 Phase-2 evidence bundle `files.txt`/`manifest.sha256`/实际文件集合一致性、`phase2_completion_audit=pass / phase2_completion_status=complete`、`phase2_completion_audit_metrics.prom`、production soak archive/evidence，以及 renderer/capture 的真实证据或 policy skip 证据。`PHASE5_DRY_RUN=1` 只验证 gate 结构，不代表 production soak 完成。
...
这个脚本已经具备生产级多轮/多分钟 soak 的执行入口和归档/验签入口；P5 默认使用 `SOAK_MINUTES=10` 完成短长稳和证据闭环。真正的“多小时”结论需要在稳定机器上显式用 `SOAK_MINUTES=120 MIN_PRODUCTION_SOAK_MINUTES=120` 或更长时间实际跑完，并把 tarball、summary 和日志归档。若要把扩展 stress 纳入专项验收，可以显式追加 `SCENARIOS="baseline weak_network_low_rps_low_bitrate walking_dead_zone_recover oscillating_edge_recover"`，但这不属于 P5 默认完成门禁。
...
这个审计脚本会读取 smoke/qoe summary、production soak summary/config/archive、real renderer summary/metrics 和 capture manifest summary，并复用 `verify_webrtc_first_qoe_production_soak_evidence.sh` 校验 sha256 manifest、summary/CSV 一致性、`SOAK_MINUTES>=10`、row pass、弱网低 RPS/低码率和恢复时间分布。P5 默认允许 real renderer 和 capture library 的 `skipped_by_policy` 证据；严格模式把 `P5_SKIP_REAL_RENDERER=0 P5_SKIP_CAPTURE_LIBRARY=0` 后，会继续用 `verify_real_renderer_evidence.sh` 校验真实 renderer status/backend/rendered frames/late/gap/jitter，并用 capture verifier 校验业务素材 QoE。它的默认 production 输入路径是 `artifacts/webrtc_first_phase2_verify_production/`，也可以通过 `EVIDENCE_BUNDLE_DIR` 审计收集好的 bundle；bundle 审计会同时复验 `files.txt`、`manifest.sha256` 和实际文件集合一致性，避免外部证据目录漏列或混入文件。每次执行都会写出 `phase2_completion_audit_metrics.prom`，即使失败也会暴露缺失证据的机器可读状态。
...
正式命令默认使用 `SOAK_MINUTES=10` 和 `PRODUCTION_SOAK_SCENARIOS="baseline weak_network_low_rps_low_bitrate"`；只有做扩展 stress 或严格生产验收时才显式追加 dead-zone/oscillating 场景或把 soak 调到 120 分钟。

如果要做严格实机/业务素材验收，把 `P5_SKIP_REAL_RENDERER=0 P5_SKIP_CAPTURE_LIBRARY=0`，并同时提供 `CAPTURE_LIBRARY_DIR=/path/to/business_capture_library CAPTURE_LIBRARY_MANIFEST=/path/to/business_capture_library/manifest.csv`。
